 ...
 * 
 * Calculator App Implementation for ESP-Brookesia Phone System
 * Adapted for ESP32-P4 JC4880P433C with modern Brookesia API
 */
#include <math.h>
#include <vector>
#include "esp_log.h"
...
bool Calculator::run(void)
{
    ...
    ESP_LOGI("Calculator", "run() completed successfully");
    return true;
}
...
void Calculator::keyboard_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_DRAW_MAIN_BEGIN) {
        ...
        if (draw_task == nullptr || draw_task->draw_dsc == nullptr) {
            return;  // Safety check: draw_task or draw_dsc is null
        }
        
        lv_draw_dsc_base_t * base_dsc = (lv_draw_dsc_base_t *)draw_task->draw_dsc;
        
        /* Color special operator buttons */
        if(base_dsc->part == LV_PART_ITEMS) {
            lv_buttonmatrix_t * btnm = (lv_buttonmatrix_t *)lv_event_get_target(e);
            uint32_t btn_id = base_dsc->id1;
            
            /*Change color for operator buttons: C,/,x,backspace,-,+,%*/
            if((btn_id == 0) || (btn_id == 1) ||
               ...
               (btn_id == 15)) {
                ...
                if(label_draw_dsc) {
                    label_draw_dsc->color = KEYBOARD_SPECIAL_COLOR;
                }
            }
        }
        ...
        double res_num;
        char res_str[64];       // Increased buffer size
        char history_str[128];  // Increased buffer size
